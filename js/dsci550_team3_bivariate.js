...
function _chart(d3,legend,data,n,colors,labels,topojson,us,states) {
  ...
  const format = (value) => {
    if (!value) return "N/A";
    if (value.sightings == 0 && value.drinking == 0) {
      return `N/A Sightings\nN/A Drinking`;
    }
    const {sightings: a, drinking: b} = value;
    ...
    () => {
      ...
      <g transform="translate(-${k * n / 2},-${k * n / 2}) rotate(-45 ${k * n / 2},${k * n / 2})">
        ...
        ${d3.cross(d3.range(n), d3.range(n)).map(([i, j]) => svg`<rect width=${k} height=${k} x=${i * k} y=${(n - 1 - j) * k} fill=${colors[j * n + i]}>
          <title>Sightings${labels[j] && ` (${labels[j]})`}\nExcessive Drinking${labels[i] && ` (${labels[i]})`}</title>
        </rect>`)}
        ...
        <text font-weight="bold" dy="0.71em" transform="translate(${n / 2 * k},${n * k + 6})" text-anchor="middle">Excessive</text>
        <text font-weight="bold" dy="1.71em" transform="translate(${n / 2 * k},${n * k + 6})" text-anchor="middle">Drinking</text>
      </g>
    </g>`;
    }
  );
...
function _data(FileAttachment) {
  return(
    FileAttachment("bfro_fips.json").json()
      .then((data) => {
        data.forEach((d) => {
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map (
    [
      ...
      ["bfro_fips.json", {url: new URL("../data/bfro_fips.json", import.meta.url), mimeType: "application/json", toString}]
    ]
  );
